    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            using (var db = new DoctorLab())
            {
                ...
                // DoctorType counts (only for doctors)
                var doctorTypeCounts = db.User_Infos
                    .Where(u => u.UserType == "Doctor")
                    .GroupBy(u => u.DocType)
                    .Select(g => new { DoctorType = g.Key, Count = g.Count() })
                    .ToList();

                // Prepare DoctorType data for the view
                var doctorTypeLabels = doctorTypeCounts.Select(x => x.DoctorType?? "Unknown").ToList();
                var doctorTypeData = doctorTypeCounts.Select(x => x.Count).ToList();

                // Group appointments by day (fix: use DbFunctions.TruncateTime for EF compatibility)
                var appointmentsPerDay = db.BookingAppointments
                    .GroupBy(a => DbFunctions.TruncateTime(a.Booking_Date))
                    .OrderBy(g => g.Key)
                    .Select(g => new { Day = g.Key, Count = g.Count() })
                    .ToList();

                // Prepare arrays for days and counts
                ViewBag.AppointmentDays = appointmentsPerDay
                    .Select(a => a.Day.HasValue ? a.Day.Value.ToString("yyyy-MM-dd") : "")
                    .ToArray();
                ViewBag.AppointmentCounts = appointmentsPerDay
                    .Select(a => a.Count)
                    .ToArray();
                ...
                ViewBag.DoctorTypeLabels = doctorTypeLabels;
                ViewBag.DoctorTypeData = doctorTypeData;
            }
        ...
        public ActionResult Payment()
        {
            if (Session["UserEmail"] == null || Session["UserType"].ToString() != "Admin" || Session["Status"].ToString() != "Active")
                return RedirectToAction("Login", "Login");
            var payments = _contextdb.Payments.ToList();
        ...
        public ActionResult Appointments()
        {
            using (var db = new DoctorLab())
            {
                var appointments = (from a in db.BookingAppointments
                                    join p in db.User_Infos on a.PatientId equals p.Id
                                    join d in db.User_Infos on a.DoctorId equals d.Id
        ...
        public ActionResult Doctor_Details()
        {
            if (Session["UserEmail"] == null || Session["UserType"].ToString() != "Admin" || Session["Status"].ToString() != "Active")
                return RedirectToAction("Login", "Login");
            ...
            using (var db = new DoctorLab())
            {
                // Eager load User_Info for each Doctor_Details
                var doctorDetails = db.Doctor_Details.Include("User_Info").ToList();
